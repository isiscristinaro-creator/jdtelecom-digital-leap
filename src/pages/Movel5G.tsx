...
import {
  ...
  CreditCard, BarChart3, Sparkles, Menu, X, User, MapPin, CheckCircle,
  ...
  >
    ...
    <div className={`rounded-2xl overflow-hidden h-full flex flex-col border transition-all ${
      ...
      <div className={`px-4 py-2 flex items-center gap-2 ${
        plan.tier === "Turbo" ? "bg-gradient-to-r from-primary/20 to-orange-500/10" : "bg-white/[0.03]"
      }`}>
        ...
        <div className="mt-auto">
          ...
          <Button
            className={`w-full rounded-xl h-11 text-sm font-bold ${
              plan.popular ? "bg-primary text-primary-foreground hover:bg-primary/90 shadow-glow" : "bg-white/10 text-white hover:bg-white/15"
            }`}
...
/* ── Plan Carousel (mobile) / Grid (desktop) ── */
  ...
  if (!isMobile) {
    ...
    return (
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 max-w-6xl mx-auto">
        {plans.map((plan, i) => (
          <PlanCard key={plan.name} plan={plan} visible={visible} index={i} navigate={navigate} />
        ))}
      </div>
    );
  }
    ...
    <div className="relative">
      ...
      {current > 0 && (
        <button
          ...
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-primary/90 text-white flex items-center justify-center shadow-lg"
        >
      ...
      {current < plans.length - 1 && (
        <button
          onClick={() => scrollToIndex(current + 1)}
          className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-primary/90 text-white flex items-center justify-center shadow-lg"
        >
...
const CoverageMapSection = () => {
  ...
  return (
    <section id="cobertura" className="py-16 sm:py-24 border-y border-white/5" ref={ref}>
      <div className="container mx-auto px-4">
        ...
        >
          <span className="inline-flex items-center gap-2 text-primary text-xs font-bold mb-4">
            <MapPin className="w-4 h-4" /> Cobertura Móvel
          </span>
          <h2 className="font-display text-2xl sm:text-4xl lg:text-5xl font-bold mb-4">
            Onde estamos com <span className="text-primary">5G</span>
          </h2>
          <p className="text-white/50 text-sm sm:text-base max-w-md mx-auto">
            Cobertura crescente na região Norte, levando conexão 5G para mais cidades
          </p>
        </motion.div>
            ...
            <div className="relative bg-white/[0.03] border border-white/[0.08] rounded-3xl p-6 md:p-10 overflow-hidden">
              ...
              <svg viewBox="0 0 100 100" className="w-full h-auto relative z-10" style={{ minHeight: 250 }}>
                ...
                {coverageCities.map((city, i) => {
                  ...
                  return coverageCities.slice(i + 1).map((city2) => {
                    ...
                    return (
                      <line key={`${city.name}-${city2.name}`} x1={pos.x} y1={pos.y} x2={pos2.x} y2={pos2.y} stroke="hsl(24 95% 50%)" strokeWidth="0.15" opacity="0.3" />
                    );
                  });
                })}
              ...
              <div className="flex items-center justify-center gap-4 mt-4 relative z-10 flex-wrap">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/40">Cobertura 5G</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-[hsl(200,60%,50%)]" />
                  <span className="text-xs text-white/40">Cobertura 4G</span>
                </div>
                <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-2">
              {coverageCities.map((city) => (
                ...
                >
                  ...
                  {city.has5g && (
                    <span className="ml-auto text-[9px] font-bold bg-primary/20 text-primary px-1.5 py-0.5 rounded-full shrink-0">5G</span>
                  )}
                </button>
              ))}
            </div>
    ...
    <div className="min-h-screen bg-[hsl(220,20%,6%)] text-white overflow-x-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
            <motion.div initial={{ opacity: 0, y: 40 }} animate={heroVisible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }}>
              <motion.span className="inline-flex items-center gap-2 bg-primary/10 border border-primary/20 text-primary text-xs font-bold px-4 py-2 rounded-full mb-6" initial={{ opacity: 0, scale: 0.8 }} animate={heroVisible ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.2 }}>
                <Sparkles className="w-3.5 h-3.5" /> Nova Era Digital
              </motion.span>
              <h1 className="font-display text-3xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold leading-[1.05] mb-6">
                A ERA DIGITAL CHEGOU,{" "}<span className="text-primary">VOCÊ ESTÁ PRONTO?</span>
              </h1>
            ...
            <motion.div className="flex justify-center lg:justify-end" initial={{ opacity: 0, scale: 0.8, rotateY: 15 }} animate={heroVisible ? { opacity: 1, scale: 1, rotateY: 0 } : {}} transition={{ duration: 1, delay: 0.3 }}>
              <div className="relative">
                <div className="absolute inset-0 bg-primary/20 blur-[80px] rounded-full" />
                <img src={heroImg} alt="5G Mobile" className="relative w-64 sm:w-80 lg:w-96 h-auto" width={800} height={800} />
                <motion.div className="absolute -top-4 -right-4 bg-primary text-primary-foreground text-xl sm:text-2xl font-black px-4 py-2 rounded-2xl shadow-glow-lg" animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity }}>
                  5G
                </motion.div>
              </div>
            </motion.div>
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            {[
              ...
              <motion.div key={f.title} className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-5 sm:p-6 hover:border-primary/30 transition-all duration-300 group" initial={{ opacity: 0, y: 30 }} animate={featVisible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: i * 0.1 }} whileHover={{ y: -4 }}>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 max-w-4xl mx-auto">
            {[
              ...
              <motion.div key={item.title} className="bg-white/[0.04] border border-white/[0.08] rounded-2xl p-6 text-center hover:border-primary/30 transition-all" initial={{ opacity: 0, y: 30 }} animate={whyVisible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: i * 0.15 }} whileHover={{ y: -4 }}>
      ...
      <footer className="py-8 border-t border-white/5">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xs text-white/30">© {new Date().getFullYear()} JD Telecom. Todos os direitos reservados.</p>
          <button onClick={() => navigate("/")} className="text-xs text-primary/60 hover:text-primary mt-2 inline-block">← Voltar ao site principal</button>
        </div>
      </footer>
